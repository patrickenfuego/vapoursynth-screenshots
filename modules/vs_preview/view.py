...
WINDOWS_KEYMAP = {
    ...
    # key 'w' to write showing frame you have on screen as png to hardisk, image is 1:1, ignores zooms in blowup
...
class Preview:
    ...
    def __init__(self, clips,
        ...
        self.validate_boolean(
            dict(fullscreen=fullscreen, play=play, slider=slider, ignore_subsampling=ignore_subsampling))
        ...
        for i, clip in enumerate(self.clips_orig):
            rgb, log = convert.toRGB(clip, matrix_in_s=self.matrix_in_s, depth=depth, kernel=self.kernel,
    ...
    def show(self):
        ...
        if self.play:
            self.play = 1  # make bint from bool
        else:
            self.play = 0
        ...
        while True:
            ...
            if key != -1:  # if a key was pressed
                try:
                    ...
                    try:
                        self.i = clip_KEYMAP.index(
                            key)  # if key was pressed that suppose to change clips, change index for clips
                    except ValueError:
                        pass
                    else:
    ...
    def show_frame(self):
        ...
        try:
            ...
            f = self.error_frame()
        if isAPI4:
            self.img = np.dstack([np.array(f[p], copy=False) for p in [2, 1, 0]])
        else:
    ...
    def select_object(self, x, y):
        ...
        if x > x1 - p and x < x1 + p:
            ...
            self.flash_object(r, [((x1 - f, y1 - f), (x2 + f - 1, y2 + f - 1)),
                                  ((x1 + f, y1 + f), (x2 - f - 1, y2 - f - 1))])
            return 'all'
        else:
            return None
    ...
    def object_step_up(self):
        if self.object == 'all':
            self.move_object(0, -self.mody_subs)
        else:
            self.move_object(0, -max(self.mody, self.mody_subs))
    ...
    def object_step_left(self):
        if self.object == 'all':
            self.move_object(-self.modx_subs, 0)
        else:
            self.move_object(-max(self.modx, self.modx_subs), 0)
    ...
    def move_object(self, x, y):
        ...
        elif self.object == 'top_line':
            new_position = (self.x1 + x, self.y1 + y)
        elif self.object == 'bottom_line':
            new_position = (self.x2 + x, self.y2 + y)
        elif self.object == 'right_line':
            new_position = (self.x2 + x, self.y2 + y)
        elif self.object == 'right_bottom_corner':
            new_position = (self.x2 + x, self.y2 + y)
        elif self.object == 'right_top_corner':
            new_position = (self.x2 + x, self.y1 + y)
        elif self.object == 'left_line':
            new_position = (self.x1 + x, self.y1 + y)
        elif self.object == 'left_bottom_corner':
            new_position = (self.x1 + x, self.y2 + y)
        elif self.object == 'left_top_corner':
            new_position = (self.x1 + x, self.y1 + y)

        if self.object == 'all':
            rectangle = self.move_rectangle(*new_position)
        else:
            rectangle = self.new_rectangle(*new_position)
        self.live_crop_info(rectangle)
        if not self.play:
            cv2.imshow(self.title, self.img_and_selection(self.img, rectangle, self.color))
    ...
    def set_window_normal(self, title):
        try:
            cv2.namedWindow(title,
                            cv2.WINDOW_NORMAL)  # strict 1:1 cv2.WINDOW_AUTOSIZE | cv2.WINDOW_KEEPRATIO or flags=cv2.WINDOW_GUI_NORMAL
    ...
    def pixel_info(self):
        ...
        # preview clip values
        info.append(
            '    preview: r:{2}  g:{1}  b:{0}'.format(*self.img[y][x]))  # tuple is returned from numpy array: (B,G,R)
        info = ''.join(info)
        self.log(info)
        if self.Qt:
    ...
    def write_image(self):
        ...
        if self.img_dir and not os.path.isdir(self.img_dir):
            ...
            return
        img_path = os.path.join(self.img_dir, self.print_clip_name() + '_{:02}_{}_frame_{:07}.png'.format(self.i + 1,
                                                                                                          self.previewData[
                                                                                                              -1],
                                                                                                          self.frame))
        '''
    ...
    def write_image_1_to_1(self):
        ...
        img_path = os.path.join(self.img_dir,
                                self.print_clip_name() + '_{:02}__1;1_{}_frame_{:07}.png'.format(self.i + 1,
                                                                                                 self.previewData[-1],
                                                                                                 self.frame))
    ...
    def validate_delay(self):
        if self.delay is None:
            ...
            try:
                ...
                raise TypeError(
                    f"[Preview] wrong delay: {self.delay} , it has to be an integer, miliseconds between frames")
    ...
    def validate_kernel(self):
        try:
            ...
            raise ValueError(
                f"[Preview] wrong kernel argument: '{self.kernel}', valid resize arguments:\n'Point','Bicubic','Lanczos','Spline16','Spline36'")
    ...
    def validate_position(self):
        ...
        if isinstance(self.position, tuple) or isinstance(self.position, list):
            if not isinstance(self.position[0], int) or not isinstance(self.position[1], int):
                raise ValueError(
                    '[Preview] position argument for Preview window must be tuple with two integers, example: position=(60,60)')

        else:
            raise ValueError(
                "[Preview] 'position' argument for Preview window must be tuple, example: position=(60,60)")
        ...
        for string in ['width', 'height']:
            ...
            if var is None:
                ...
                if not (isinstance(var, int) and var > 1 and var <= 20000):
                    raise ValueError(
                        f"[Preview] 'preview_{string}' argument must be positive integer and less than 20000")
    ...
    def validate_mod(self, modx, mody):
        ...
        if not isinstance(modx, int):
            raise TypeError(
                f"[Preview] wrong 'mod_x' argument: {mod_x}, it has to be an integer, usual values are 2,4,8 or 16")
        if not isinstance(mody, int):
            raise TypeError(
                f"[Preview] wrong 'mod_y' argument: {mod_y}, it has to be an integer, usual values are 2,4,8 or 16")
        modx = abs(modx)
        mody = abs(mody)
        old_modx = modx
        old_mody = mody
        ...
        if respect_x_subs and respect_y_subs:
            if modx != old_modx: self.log(
                f'  mod_x={modx},  that is correction of  mod_x={old_modx}, cannot be less than subsampling')
            if mody != old_mody: self.log(
                f'  mod_y={mody},  that is correction of  mod_y={old_mody}, cannot be less than subsampling')
            ...
            mody_subs = 1
            if modx != old_modx: self.log(
                f'  mod_x={modx},  that is correction of  mod_x={old_modx}, because it must be >= {modx}')

        elif not respect_x_subs and not respect_y_subs:
            modx_subs, mody_subs = 1, 1

        self.log(
            f'  subsampling mods:      {modx_subs}, {mody_subs}   while selecting crop area, first selected point snaps to those mods')
        self.log(
            f'  video resolution mods: {modx}, {mody}   while drawing crop area, width and height are snapping to those mods')
    ...
    def freeRAM(self):
        ...
        self.log(
            "[freeRAM] psutil is needed to get free RAM on Mac")  # or add some fallback code for Mac here that works
...
class Conversions:
    ...
    def getMatrix(self, clip=None, matrix_in_s=None):
        ...
        if matrix_in_s:
            ...
            if clip.width <= 1024 and clip.height <= 480:
                matrix_in, matrix_in_s = (6, '170m')
            elif clip.width <= 1024 and clip.height <= 576:
                matrix_in, matrix_in_s = (5, '470bg')
            else:
                matrix_in, matrix_in_s = (1, '709')
    ...
    def toRGB(self, clip=None, matrix_in_s=None, depth=None, kernel=None, sample_type=None):
        ...
        if matrix_in_s:
            inMatrix = "matrix_in_s = '{}',".format(matrix_in_s)
        else:
            inMatrix = ''
        if clip.format.sample_type == 0:
            inSample = 'INTEGER'
        elif clip.format.sample_type == 1:
            inSample = 'FLOAT'
        if sample_out == vs.INTEGER:
            outSample = 'vs.INTEGER'
        else:
            outSample = 'vs.FLOAT'

        # zimg conversion
        try:
            log += '[toRGB] zimg conversion, {}bit {} {} to {}bit {} RGB:\n'.format(depth_in, inSample, name, depth,
                                                                                    outSample)
            if matrix_in_s:
                clip = _resize(clip, **format_out, matrix_in_s=matrix_in_s)
            else:
                clip = _resize(clip, **format_out)
